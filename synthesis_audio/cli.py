...
def synthesis():
    ...
    for paras in [f"text_translated/{doc}" for doc in os.listdir("text_translated")]:
        ...
        audio_file = f"{output_audios}/{paras.split('/')[1][:-1*len('.txt')]}.mp3"
...
def upload():
    ...
    blob_files = os.listdir(output_audios)
    destination_blob_names = [f"{output_audios}/{blob_file}" \
